...
const main = () => {
	...
	for (let i = 7; i >= 0; i--) {
		...
		shapes.push(colorize(shapeColor, translate([(i - 3) * 7.5, 0, 0], cylinder({ radius: 3, height: 20 }))));
	}
	shapes.push(colorize(colorNameToRgb("red") as RGB, translate([-4, -10, 0], cuboid({ size: [5, 5, 5] }))));
	shapes.push(colorize([1, 0, 0, 0.5], translate([4, -10, 0], cuboid({ size: [5, 5, 5] }))));
	return shapes;
};
