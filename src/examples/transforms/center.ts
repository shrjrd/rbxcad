 ...
 * @returns {geometry}
 */
const main = (params: { centerx: boolean; centery: boolean; centerz: boolean }) => {
	let crossHair3D = crosshair();
	if (params.centerx || params.centery || params.centerz) {
		crossHair3D = center({ axes: [params.centerx, params.centery, params.centerz] }, crossHair3D) as Geom3;
	}
	return crossHair3D;
};
