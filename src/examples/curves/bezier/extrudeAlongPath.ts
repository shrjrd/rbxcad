...
const tube = (bezierControlPoints: Vec3[]) => {
	...
	return extrusions.extrudeFromSlices(
		{
			...
			callback: (progress, count, base) => {
				const positionArray = bezier.valueAt(progress, tubeCurve) as Vec3;
				const tangentArray = bezier.tangentAt(progress, tubeCurve) as Vec3;
...
const rotationMatrixFromVectors = (srcVector: Vec3, targetVector: Vec3) => {
...
const box4x4 = (translation: Vec3, color: RGB) => {
