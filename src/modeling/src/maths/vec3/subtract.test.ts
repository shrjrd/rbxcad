...
test("vec3: subtract() called with three parameters should update a vec3 with correct values", () => {
	...
	expect(compareVectors(obs2, [-2, 0, 2])).toBe(true);
	expect(compareVectors(ret2, [-2, 0, 2])).toBe(true);

	const obs3 = fromValues(0, 0, 0);
	const ret3 = subtract(obs3, [1, 2, 3], [-1, -2, -3]);
	expect(compareVectors(obs3, [2, 4, 6])).toBe(true);
	expect(compareVectors(ret3, [2, 4, 6])).toBe(true);

	const obs4 = fromValues(0, 0, 0);
	const ret4 = subtract(obs4, [-1, -2, -3], [-1, -2, -3]);
	expect(compareVectors(obs4, [0, 0, 0])).toBe(true);
	expect(compareVectors(ret4, [0, 0, 0])).toBe(true);
});
