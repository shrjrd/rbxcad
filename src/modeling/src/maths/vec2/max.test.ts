...
test("vec2: max() called with three parameters should update a vec2 with correct values", () => {
	...
	expect(compareVectors(obs2, [1, 1])).toBe(true);
	expect(compareVectors(ret2, [1, 1])).toBe(true);

	const obs3 = fromValues(0, 0);
	const vec3 = fromValues(0, 1);
	const ret3 = max(obs3, vec0, vec3);
	expect(compareVectors(obs3, [0, 1])).toBe(true);
	expect(compareVectors(ret3, [0, 1])).toBe(true);
