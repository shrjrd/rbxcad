...
import { Node, removeNode } from "./linkedList";
import { linkedListSort } from "./linkedListSort";
 ...
 *
 * @param {number[]} data A flat array of vertex coordinates.
 * @param {number[]} holeIndices An array of hole indices if any.
 * @param {number} dim The number of coordinates per vertex in the input array.
 */
export const triangulate = (data: number[], holeIndices: number[], dim = 2) => {
	const hasHoles = holeIndices && holeIndices.size();
	// DEVIATION: 0, NaN, and "" are falsy in TS.
	const outerLen = hasHoles > 0 ? holeIndices[0] * dim : data.size();
...
const earcutLinked = (
	...
	// DEVIATION: 0, NaN, and "" are falsy in TS.
	if ((pass === 0 || pass === undefined) && invSize) indexCurve(ear, minX, minY, invSize);
