...
import type { Vec2, Line2 } from "../../maths/types";
...
import { insertSorted } from "../../utils/insertSorted";
 ...
 */
export const reTesselateCoplanarPolygons = (sourcePolygons: Poly3[]) => {
	...
	for (let yIndex = 0; yIndex < yCoordinates.size(); yIndex++) {
		...
		let nextYcoordinate: number = undefined!;
		if (yIndex >= yCoordinates.size() - 1) {
			...
			if (startingPolygonIndexes) {
				//for (const polygonIndexKey of startingPolygonIndexes) {
		...
		for (const activePolygon of activePolygons) {
			...
			if (newOutPolygonRow.size() > 0) {
				...
				if (d1 < EPS && d2 < EPS) {
					...
					JsArray.splice(newOutPolygonRow, newOutPolygonRow.size() - 1 + 1, 1); //newOutPolygonRow.splice(newOutPolygonRow.length - 1, 1);
				}
			}
