import { Array, JsMap, Number, Object } from "@rbxts/luau-polyfill";
 ...
 */
const offsetFromPoints = (options: OffsetFromPointsOptions, points: Vec2[]): Vec2[] => {
	const defaults = {
		delta: 1,
		corners: "edge",
		closed: false,
	...
	if (closed && previousSegment !== undefined) {
	...
	if (corners === "edge") {
		...
		const pointIndex = new JsMap<Vec2, number>(); // {point: index}
		...
		newCorners.forEach((corner) => {
			...
			if (Number.isFinite(ip[0]) && Number.isFinite(ip[1])) {
				...
				const i = pointIndex.get(p0) as number;
				newPoints[i] = ip;
				newPoints[(i + 1) % newPoints.size()] = undefined!;
			} else {
		...
		newCorners.forEach((corner) => {
			...
			if (rotation !== 0.0) {
				...
				if (cornerpoints.size() > 0) {
					...
					i = (i + 1) % newPoints.size();
					Array.splice(newPoints, i + 1, 0, ...cornerpoints); //newPoints.splice(i, 0, ...cornerpoints);
				}
			} else {
				// paralell segments, drop one
				const p0 = corner.s1[0];
				const i = newPoints.findIndex((point) => vec2.equals(p0, point));
				Array.splice(newPoints, i + 1, 1); //newPoints.splice(i, 1);
			}
		});
	}
	return newPoints;
