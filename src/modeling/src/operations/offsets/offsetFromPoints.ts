...
import * as line2 from "../../maths/line2/index";
 ...
 */
export const offsetFromPoints = (options: OffsetFromPointsOptions, points: Vec2[]) => {
	const defaults = {
		delta: 1,
		corners: "edge",
		closed: false,
	...
	if (corners === "edge") {
		...
		const pointIndex = new Map<Vec2, number>(); // {point: index}
		...
		newCorners.forEach((corner) => {
			...
			if (rotation !== 0.0) {
				...
				if (cornerPoints.size() > 0) {
					...
					JsArray.splice(newPoints, i + 1, 0, ...cornerPoints); //newPoints.splice(i, 0, ...cornerPoints);
				}
			} else {
				// parallel segments, drop one
				const p0 = corner.s1[0];
				const i = newPoints.findIndex((point) => vec2.equals(p0, point));
				JsArray.splice(newPoints, i + 1, 1); //newPoints.splice(i, 1);
			}
		});
	}
	return newPoints;
