...
import { center, translate } from "../transforms/index";
import { union } from "./index";

test("union of one or more geom2 objects produces expected geometry", () => {
	const geometry1 = circle({ radius: 2, segments: 8 });

	// union of one object
	const result1 = union(geometry1) as Geom2;
	...
	const result2 = union(geometry1, geometry2) as Geom2;
	...
	expect(measureArea(result2)).toBe(27.31370849898476);
	...
	const result3 = union(geometry2, geometry3) as Geom2;
	...
	expect(measureArea(result3)).toBe(339);
	expect(obs.size()).toBe(8);
	expect(comparePoints(obs, exp)).toBe(true);

	// union of two completely overlapping objects
	const result4 = union(geometry1, geometry3) as Geom2;
	...
	expect(measureArea(result4)).toBe(324);
	...
	const result5 = union(
		union(translate([17, 21], circ), translate([7, 0], circ)) as Geom2,
		union(translate([3, 21], circ), translate([17, 21], circ)) as Geom2,
	) as Geom2;
	obs = geom2.toPoints(result5);
	expect(() => geom2.validate(result5)).never.toThrow();
	expect(measureArea(result5)).toBe(9.36433545677411);
	expect(obs.size()).toBe(96);
});

test("union of geom2 with closing issues #15", () => {
	...
	const result = union(c, d) as Geom2;
	...
	expect(measureArea(result)).toBe(17.56120670215806);
	expect(pts.size()).toBe(20); // number of sides in union
	expect(comparePoints(pts, exp)).toBe(true);
});

test("union of geom2 with colinear edge (martinez issue #155)", () => {
	...
	const result = union(g1, g2) as Geom2;
	const pts = geom2.toPoints(result);
	expect(() => geom2.validate(result)).never.toThrow();
	expect(measureArea(result)).toBe(1.9906657858562764);
	expect(pts.size()).toBe(5);
	expect(comparePoints(pts, exp)).toBe(true);
});

test("union with undefined/null values", () => {
	...
	const obs = union(...geometries) as Geom2;
	const pts = geom2.toPoints(obs);
	expect(() => geom2.validate(obs)).never.toThrow();
	expect(pts.size()).toBe(4);
});

test("union of nested arrays", () => {
	const square1 = square({ size: 8 });
	const square2 = square({ size: 6 });
	const square3 = square({ size: 4 });
	const geometries = [square1, [square2, [square3]]] as RecursiveArray<Geom2>;
