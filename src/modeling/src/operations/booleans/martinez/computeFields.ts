...
import { DIFFERENT_TRANSITION, NON_CONTRIBUTING, NORMAL, SAME_TRANSITION } from "./edgeType";
 ...
 */
export const computeFields = (event: SweepEvent, prev: SweepEvent, operation: 0 | 1 | 2 | 3) => {
...
const inResult = (event: SweepEvent, operation: 0 | 1 | 2 | 3) => {
	switch (event.type) {
		case NORMAL:
			switch (operation) {
				case INTERSECTION:
...
const determineResultTransition = (event: SweepEvent, operation: 0 | 1 | 2 | 3) => {
	...
	switch (operation) {
		...
		case UNION:
			...
			break;
		case XOR:
			isIn = thisIn !== thatIn; //thisIn ^ thatIn;
			break;
		case DIFFERENCE:
			if (event.isSubject) {
				isIn = thisIn && !thatIn;
