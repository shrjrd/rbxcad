...
//const processPolygon = (contourOrHole: Vec2[],	isSubject: boolean,	depth: number,	queue: Queue,	bbox: number[],	isExteriorRing: boolean,) => {
const processPolygon = (
	...
	const len = contourOrHole.size() - 1;
	for (let i = 0; i < len; i++) {
...
export const fillQueue = (
	...
	for (let i = 0; i < subject.size(); i++) {
		...
		for (let j = 0; j < polygonSet.size(); j++) {
