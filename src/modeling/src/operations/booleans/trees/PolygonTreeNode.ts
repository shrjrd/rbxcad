...
class PolygonTreeNode {
	...
	constructor(parent?: PolygonTreeNode, polygon?: Poly3) {
		this.parent = parent!;
		this.children = [];
		...
		for (i = 0; i < queue.size(); ++i) {
			...
			for (j = 0, l = children.size(); j < l; j++) {
				...
				if (node.polygon) {
					...
					if (node.children.size() > 0) queue.push(node.children);
				}
			}
		}
	}
	...
	toString() {
		...
		for (i = 0; i < queue.size(); ++i) {
			// queue size can change in loop, don't cache length
			children = queue[i];
			const prefix = string.rep(" ", i); //" ".repeat(i);
			for (j = 0, l = children.size(); j < l; j++) {
				// ok to cache length
				node = children[j];
