...
class QuickHull {
	...
	removeAllVerticesFromFace(face: Face) {
		if (face.outside) {
			...
			//          |           |      |
			//          a           a      b
			let _end = face.outside;
			while (_end.next && _end.next.face === face) {
				_end = _end.next;
			}
			this.claimed.removeChain(face.outside, _end);
			//                            b
			//                       [ outside, ...]
			//                            |  removes this link
			//     [ outside, ..., end ] -┘
	...
	resolveUnclaimedPoints(newFaces: Face[]) {
		...
		for (let vertex = vertexNext; vertex; vertex = vertexNext) {
			...
			let maxFace;
			for (let i = 0; i < newFaces.size(); i += 1) {
	...
	computeExtremes() {
		...
		for (i = 1; i < this.vertices.size(); i += 1) {
	...
	createInitialSimplex() {
		...
		maxDistance = 0;
		for (i = 0; i < this.vertices.size(); i += 1) {
			const vertex = this.vertices[i];
	...
	collectFaces(skipTriangulation: boolean) {
		...
		for (let i = 0; i < this.faces.size(); i += 1) {
			...
			if (skipTriangulation) {
				...
				for (let j = 0; j < indices.size() - 2; j += 1) {
	 ...
	 * ccw order
	 */
	computeHorizon(eyePoint: Vec3, crossEdge: HalfEdge, face: Face, horizon: HalfEdge[]) {
	...
	addNewFaces(eyeVertex: Vertex, horizon: HalfEdge[]) {
		...
		for (let i = 0; i < horizon.size(); i += 1) {
	...
	doAdjacentMerge(face: Face, mergeType: number) {
		...
		do {
			...
			if (merge) {
				...
				for (let i = 0; i < discardedFaces.size(); i += 1) {
	...
	addVertexToHull(eyeVertex: Vertex) {
		...
		this.computeHorizon(eyeVertex.point, undefined!, eyeVertex.face, horizon);
